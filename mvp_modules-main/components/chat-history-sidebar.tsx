...
export function ChatHistorySidebar({
  ...
  return (
    <div
      className={`flex flex-col h-full border-r bg-white transition-all duration-300 ${isCollapsed ? "w-12" : "w-64"
        }`}
    >
      {/* 头部 - 折叠按钮 */}
                    ...
                    {isExpanded && (
                      <div className="space-y-1 mt-1">
                        {groupSessions.map((session) => (
                          <div
                            key={session.id}
                            className={`group flex items-center gap-2 px-3 py-2 rounded-md cursor-pointer transition-all duration-150 ${currentSessionId === session.id
                                ? "bg-gray-100 border-l-3 border-gray-400"
                                : "hover:bg-gray-50 border-l-3 border-transparent"
                              }`}
                            onClick={() => onSessionSelect(session.id)}
                          >
                            <MessageSquare className={`h-4 w-4 flex-shrink-0 ${currentSessionId === session.id
                                ? "text-gray-600"
                                : "text-gray-400"
                              }`} />
                            <span className={`flex-1 text-sm truncate min-w-0 ${currentSessionId === session.id
                            ...
                            <button
                              className={`p-1 hover:bg-gray-200 rounded flex-shrink-0 transition-opacity ${currentSessionId === session.id
                                  ? "opacity-100"
